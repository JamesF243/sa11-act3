def parse_invoices(invoice_data)
  invoice_data.each_line do |line|
    ...
    # checks the date formatting first, before allowing a word with chars or nums
    # then another bit for the client, allowing any char (except \n)
    # and then the last bit, which has to be digits and starts with a $
    if match_data
      date = match_data[1]
